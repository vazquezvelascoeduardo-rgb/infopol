  ...
  return (
    <div className="screen">
      ...
      <div style={{ padding: '14px 0 8px' }}>
        <SectionHead kicker="Última hora" kickerColor={T.cat.operativa.solid} title="Notícies del dia" action="Tot →" />
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {news.length === 0 && (
            <div style={{ fontSize: 12, color: T.inkMuted, padding: '8px 0' }}>Carregant notícies…</div>
          )}
          {news.map((n) => {
            const tagColor = TAG_COLORS[n.tag] || T.cat.operativa.solid;
            const scopeLabel = SCOPE_LABELS[n.scope] || n.scope;
            const card = (
              <div key={n.id} style={{ background: '#fff', borderRadius: T.r.md, padding: 14, borderLeft: `2px solid ${tagColor}`, boxShadow: T.shadow.card }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: tagColor, letterSpacing: 0.6, textTransform: 'uppercase', background: `${tagColor}15`, padding: '2px 6px', borderRadius: 4 }}>{n.tag}</span>
                  {n.scope && (
                    <span style={{ fontSize: 9.5, fontWeight: 700, color: '#fff', background: tagColor, padding: '2px 5px', borderRadius: 4, opacity: 0.75 }}>{scopeLabel}</span>
                  )}
                  <span style={{ fontFamily: T.fontMono, fontSize: 10, color: T.inkMuted, marginLeft: 'auto' }}>{n.dateLabel || n.date}</span>
                </div>
                <div style={{ fontWeight: 700, fontSize: 13.5, color: T.ink, lineHeight: 1.3 }}>{n.title}</div>
                <div style={{ fontSize: 11.5, color: T.inkMuted, marginTop: 3, lineHeight: 1.4 }}>{n.desc}</div>
                {n.url && (
                  <div style={{ fontSize: 11, color: tagColor, marginTop: 6, fontWeight: 700 }}>Llegir notícia →</div>
                )}
              </div>
            );
            return n.url
