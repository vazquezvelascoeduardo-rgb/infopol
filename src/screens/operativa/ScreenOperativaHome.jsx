...
const CAT_COLORS = {
  politica: 'leyes', economia: 'academia', cultura: 'psico',
  internacional: 'transito', esports: 'atajos', policial: 'operativa',
};
    ...
    <div className="screen">
      ...
      <div style={{ padding: '14px 0 0' }}>
        <SectionHead
          ...
          title="Última hora"
          action="Tot →"
          onAction={() => navigate('/operativa/noticies')}
        />
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {news.map(n => {
            const tokenKey = CAT_COLORS[n.cat] || 'operativa';
            const k = T.cat[tokenKey];
            return (
              <div
                key={n.id}
                style={{ background: '#fff', borderRadius: T.r.md, padding: 14, borderLeft: `2px solid ${k.solid}`, boxShadow: T.shadow.card }}
        ...
        <div style={{ padding: '10px 16px 0' }}>
          <button
            onClick={() => navigate('/operativa/noticies')}
            style={{
              width: '100%', padding: '12px', borderRadius: T.r.md,
              background: T.cat.operativa.soft, border: 'none', cursor: 'pointer',
              color: T.cat.operativa.ink, fontFamily: T.font, fontWeight: 700, fontSize: 13,
            }}
          >
