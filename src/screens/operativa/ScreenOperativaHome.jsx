    ...
    <div className="screen">
      ...
      {NOTICIAS.length > 0 && (
        <div style={{ padding: '14px 0 0' }}>
          <SectionHead kicker="Noticias" kickerColor={T.cat.atajos.solid} title="Notícies del dia" action="Tot →" />
          <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {NOTICIAS.slice(0, 10).map((n, i) => (
              <div key={i}
                   onClick={n.url ? () => window.open(n.url, '_system') : undefined}
                ...
                {n.url && (
                  <div style={{ fontSize: 10.5, color: T.cat.atajos.solid, marginTop: 5, fontWeight: 700 }}>Llegir complet →</div>
                )}
              </div>
            ))}
          </div>
