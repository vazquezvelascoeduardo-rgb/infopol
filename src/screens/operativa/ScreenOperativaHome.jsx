...
import { NEWS as ALL_NEWS, NEWS_CATS } from '../../data/news';
  ...
  return (
    <div className="screen">
      ...
      <div style={{ padding: '14px 0 0' }}>
        <SectionHead kicker="Actualitat" kickerColor={T.cat.operativa.solid} title="Última hora" action="Tot →" onAction={() => navigate('/operativa/noticies')} />
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {RECENT_NEWS.map(n => {
            const cat = NEWS_CATS[n.cat] || NEWS_CATS.normatiu;
            return (
              <div key={n.id} style={{ background: '#fff', borderRadius: T.r.md, padding: 14, borderLeft: `3px solid ${cat.solid}`, boxShadow: T.shadow.card }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: cat.solid, letterSpacing: 0.6, textTransform: 'uppercase' }}>{n.tag || cat.label}</span>
                ...
                {n.url && (
                  <a href={n.url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 3, marginTop: 6, color: cat.solid, fontWeight: 700, fontSize: 11.5, textDecoration: 'none' }}>
                    Llegir <Icon name="arrow-right" size={12} color={cat.solid} />
                  </a>
                )}
              </div>
            );
