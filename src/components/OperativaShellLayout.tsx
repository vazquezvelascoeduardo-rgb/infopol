...
function NavItem({ n, on, onClick }: { n: typeof NAV[number]; on: boolean; onClick: () => void }) {
  return <button onClick={onClick} className="a-navitem" style={{ border: on ? `1px solid ${A.line}` : '1px solid transparent', cursor: 'pointer', textAlign: 'left', background: on ? A.card : 'transparent', boxShadow: on ? A.shadow : 'none', borderRadius: 14, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 12 }}>
    <span style={{ width: 30, height: 30, borderRadius: 10, flexShrink: 0, background: on ? OP : A.bgDeep, display: 'grid', placeItems: 'center', boxShadow: on ? A.inset : 'none' }}><Ic name={n.icon} size={17} color={on ? '#fff' : A.inkSoft} sw={2.2} /></span>
    <span style={{ flex: 1 }}>
